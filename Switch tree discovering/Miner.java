...
public class Miner {
	...
	public static boolean mineSwitch = true;
	public static ArrayList<Pair<XEventClass, XEventClass>> switchTransitions = new ArrayList<>(); 
	...
	public static Node mineNode(IMLog log, ProcessTree tree, MinerState minerState) {
		...
		//We need to revert the switchTransitions to previous versions if we abort the cut
		ArrayList<Pair<XEventClass, XEventClass>> switchTransitions_backup = (ArrayList<Pair<XEventClass, XEventClass>>) switchTransitions.clone();
		
		int numActivityBefore = XEventClasses.deriveEventClasses(log.getClassifier(), log.toXLog()).size();
		IMLog clone_log = log.clone();
		
		//find cut
		Cut cut = findCut(log, logInfo, minerState);
		//System.out.println("after size: " + log.size());
		if (minerState.isCancelled()) {
			return null;
		}
		...
		if (cut != null && cut.isValid()) {
			...
			int numActivityAfter = 0;
			for (IMLog sublog : splitResult.sublogs) {
				numActivityAfter = numActivityAfter + XEventClasses.deriveEventClasses(sublog.getClassifier(), sublog.toXLog()).size();
			}
			
			//if(cut.getOperator() == Operator.xor) {
			if(numActivityBefore != numActivityAfter && cut.getOperator() == Operator.xor) {
				switchTransitions = switchTransitions_backup;
				//System.out.println("before: " + numActivityBefore);
				//System.out.println("after " + numActivityAfter);
				System.out.println("Abort");
				mineSwitch = false;
				System.out.println("before: " + numActivityBefore);
				System.out.println("after: " + numActivityAfter);
				log = clone_log;
				cut = findCut(log, logInfo, minerState);
				mineSwitch = true;
				if (cut != null && cut.isValid()) {
					splitResult = splitLog(log, logInfo, cut, minerState);
				}
