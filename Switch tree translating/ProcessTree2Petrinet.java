package org.processmining.processtree.conversion;
...
/**
 * 
 * @see PTConversion package
 *
 */
@Deprecated
public class ProcessTree2Petrinet {

	@Plugin(name = "Convert Process Tree to Petri Net 2", level = PluginLevel.PeerReviewed, returnLabels = {
			"Petri net", "Initial Marking", "Final Marking" }, returnTypes = { Petrinet.class, Marking.class,
					Marking.class }, parameterLabels = { "Process Tree" }, userAccessible = true)
	...
	public Object[] convert(PluginContext context, ProcessTree tree)
		...
		PetrinetWithMarkings result = convert(tree, false);
	...
	@Plugin(name = "Convert Process Tree to Petri Net - keep structure", returnLabels = { "Petri net",
			"Initial Marking", "Final Marking" }, returnTypes = { Petrinet.class, Marking.class,
					Marking.class }, parameterLabels = { "Process Tree" }, userAccessible = true)
	@UITopiaVariant(affiliation = UITopiaVariant.EHV, author = "S.J.J. Leemans", email = "s.j.j.leemans@tue.nl")
	@PluginVariant(variantLabel = "Convert Process Tree to Petri Net, default", requiredParameterLabels = { 0 })
	public Object[] convertKeepStructure(PluginContext context, ProcessTree tree)
			throws NotYetImplementedException, InvalidProcessTreeException {
		PetrinetWithMarkings result = convert(tree, true);
	...
	public static PetrinetWithMarkings convert(ProcessTree tree, boolean keepStructure)
		...
		System.out.println("Please enter switch behaviours (Activity1, Activity2) line by line, type finish when you complete");
		...
		while(keyboard.hasNextLine()) {
			...
			Transition a = null;
			Transition b = null;
			for(Transition i:petrinet.getTransitions()) {
				if(i.getLabel().equals(input.split(", ")[0])) {
					a = i;
				}
				if(i.getLabel().equals(input.split(", ")[1])) {
					b = i;
				}
			}
			if(a == null ||b == null) {
			...
			PetrinetNode node1 = petrinet.getOutEdges(a).iterator().next().getTarget();
			if(petrinet.getInEdges(node1).size() > 1) {
				boolean end = true;
				Iterator<PetrinetEdge<? extends PetrinetNode, ? extends PetrinetNode>> i = petrinet.getInEdges(node1).iterator();
				while(i.hasNext()) {
					if(i.next().getSource().getLabel().equals("switch")) {
						end = false;
						break;
					}
				}
				if(end) {
					Transition bridge = petrinet.addTransition("bridge");
					bridge.setInvisible(true);
					petrinet.removeArc(a, node1);
					petrinet.addArc(bridge, (Place) node1);
					Place p = petrinet.addPlace("p");
					petrinet.addArc(a, p);
					petrinet.addArc(p, bridge);
					node1 = p;
				}
				
			}
			PetrinetNode node2 = petrinet.getInEdges(b).iterator().next().getSource();
			if(petrinet.getOutEdges(node2).size() > 1) {
				boolean start = true;
				Iterator<PetrinetEdge<? extends PetrinetNode, ? extends PetrinetNode>> i = petrinet.getOutEdges(node2).iterator();
				while(i.hasNext()) {
					if(i.next().getTarget().getLabel().equals("switch")) {
						start = false;
						break;
					}
				}
				if(start) {
					Transition bridge = petrinet.addTransition("bridge");
					bridge.setInvisible(true);
					petrinet.removeArc(node2, b);
					Place p = petrinet.addPlace("p");
					petrinet.addArc((Place) node2, bridge);
					petrinet.addArc(bridge, p);
					petrinet.addArc(p, b);
					node2 = p;
				}
				
			}
			Transition t = petrinet.addTransition("switch");
			t.setInvisible(true);
			petrinet.addArc((Place) node1, t);
			petrinet.addArc(t, (Place) node2);
